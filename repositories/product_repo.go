...
func (p *ProductRepository) FindAllProducts(ctx context.Context) ([]model.Product, error) {
	...
	err := p.db.WithContext(ctx).Table("products").Select("id, name, price, stock, discount, type").Where("deleted_at is null").Find(&results).Error
	if err != nil {
...
func (p *ProductRepository) FindProductByID(ctx context.Context, id string) (model.Product, error) {
	...
	result := p.db.WithContext(ctx).Table("products").
		Select("id, name, price, stock, discount, type, seller_id").Where("deleted_at is null").
		First(&product, "id = ?", id)
	if result.Error != nil {
...
func (p *ProductRepository) FindProductBySellerID(ctx context.Context, sellerId string) ([]model.Product, error) {
	var product []model.Product
	result := p.db.WithContext(ctx).Table("products").
		Select("id, name, price, stock, discount, type").Where("deleted_at is null").
		First(&product, "seller_id = ?", sellerId)
